...
/* Handset, drawn in currentColor so it takes the button’s own text colour in
   either theme rather than needing a light and a dark fill of its own. */
function PhoneGlyph() {
    return (
        <svg viewBox="0 0 24 24" aria-hidden="true" fill="currentColor" className="size-4 shrink-0">
            <path d="M6.62 10.79a15.1 15.1 0 0 0 6.59 6.59l2.2-2.2a1 1 0 0 1 1.02-.24c1.12.37 2.33.57 3.57.57a1 1 0 0 1 1 1V20a1 1 0 0 1-1 1C10.61 21 3 13.39 3 4a1 1 0 0 1 1-1h3.5a1 1 0 0 1 1 1c0 1.25.2 2.45.57 3.57a1 1 0 0 1-.25 1.02l-2.2 2.2Z" />
        </svg>
    );
}
            ...
            <Container className="flex flex-col gap-5 pb-[calc(1.75rem+env(safe-area-inset-bottom))] pt-2">
                <div style={{ transitionDelay: open ? `${nav.length * ITEM_STEP}ms` : "0ms" }}
                    ...
                    <a href={`tel:${site.phones[0].tel}`}
                        className="flex items-center justify-center gap-3 rounded-2xl bg-champagne-300 px-6 py-4 font-display text-sm uppercase tracking-luxe text-navy-950 shadow-[0_10px_30px_-12px] shadow-champagne-400/70 transition-transform duration-200 active:scale-[0.98]">
                        <PhoneGlyph />
